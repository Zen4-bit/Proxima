...
class IPCClient {
    ...
    async send(action, provider = null, data = {}) {
        ...
        return new Promise((resolve, reject) => {
            ...
            // Timeout after 5 minutes (image generation can take 2-3 min)
            setTimeout(() => {
...
const PROVIDERS_CACHE_TTL = 30_000; // re-read file at most every 30s

function getEnabledProviders() {
    const now = Date.now();
    if (_providersCache && now - _providersCacheTime < PROVIDERS_CACHE_TTL) {
    ...
    try {
        ...
        if (fs.existsSync(appDataPath)) {
            ...
            _providersCache = new Set(data.enabled || []);
    ...
    _providersCache = new Set(['perplexity', 'chatgpt', 'gemini']);
    _providersCacheTime = now;
    return _providersCache;
}
...
class AIProvider {
    ...
    async chat(message, useCache = true) {
        ...
        // Small delay so Electron DOM can start rendering the new response
        // before getResponseWithTyping reads it — prevents off-by-one stale response
            ...
            await new Promise((resolve, reject) => {
                ...
                httpModule.get(imageUrl, (response) => {
                    if (response.statusCode >= 300 && response.statusCode < 400 && response.headers.location) {
                        const redirectModule = response.headers.location.startsWith('https') ? https : http;
...
server.tool(
    'generate_image',
    `Generate a single image via ChatGPT's built-in image generator.

IMPORTANT RULES — follow strictly:
1. ALWAYS call new_conversation before generating an image to start a fresh chat. This prevents context bleed and stale image returns.
2. Generate ONE image at a time. Never call this tool in parallel. Wait for the result before generating the next image.
3. Be patient — this tool blocks for 30–90 seconds while the image generates. The tool will return automatically when done. Do NOT retry while waiting.
4. If the tool returns an error or empty result, call new_conversation and try once more with a revised prompt.

NEW CONVERSATION FLOW:
  Step 1: call new_conversation   ← resets ChatGPT chat context
  Step 2: call generate_image     ← generates image in fresh chat
  For multiple images: new_conversation → generate_image → new_conversation → generate_image → ...

STYLE CONSISTENCY (multiple images):
  - If you have a reference image (local file path), pass it via reference_image. ChatGPT will use it for style matching.
  - If no reference image: describe the style very explicitly in every prompt (same color palette, same art style wording, same character description).

PROMPT STRUCTURE — build prompts like this:
  [subject], [action/pose], [setting/background], [lighting], [style], [quality modifiers]
  Example: "a red fox sitting on a snowy hill, golden hour lighting, photorealistic, detailed fur, 4k"
  - Be specific and descriptive. Vague prompts produce generic results.
  - Include style if relevant (e.g. "pixel art", "oil painting", "anime", "photorealistic").
  - Include size/aspect ratio via the size parameter, not in the prompt text.`,
    {
        prompt: z.string().describe('Image description. Be specific: subject, setting, lighting, style, quality. Example: "a red fox on a snowy hill, golden hour, photorealistic, 4k"'),
        style: z.string().optional().describe('Art style override (e.g. photorealistic, oil painting, pixel art, anime, cyberpunk, watercolor)'),
        size: z.string().optional().describe('Aspect ratio hint: "square" (1:1), "landscape" (16:9), "portrait" (9:16)'),
        reference_image: z.string().optional().describe('Absolute path to a local image file to use as style/content reference. ChatGPT will generate a new image matching the style of this reference.')
    },
    async ({ prompt, style, size, reference_image }) => {
        const disabled = checkDisabled('chatgpt');
        if (disabled) return disabled;
        try {
            const styleStr = style ? `, ${style} style` : '';
            const sizeStr = size ? `, ${size} format` : '';
            const fullPrompt = `Generate an image with this description: ${prompt}${styleStr}${sizeStr}. Output only the image, no explanation.`;
            if (reference_image && fs.existsSync(reference_image)) {
                return toolResponse(await chatgpt.chatWithFile(fullPrompt, reference_image));
            }
            return toolResponse(await chatgpt.chat(fullPrompt, false));
        } catch (err) {
            return toolError(err);
        }
    }
